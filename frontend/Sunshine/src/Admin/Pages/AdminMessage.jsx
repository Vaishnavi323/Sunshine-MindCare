...
const AdminMessages = () => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      if (result.status && result.data) {
        ...
        const transformedMessages = transformMessagesData(result.data);
  ...
  // Transform API response to match your component structure
  const transformMessagesData = (apiData) => {
    return apiData
      .filter(message => 
      ...
      .map(message => ({
        id: message.id,
        name: `${message.first_name || ''} ${message.last_name || ''}`.trim() || 'Anonymous User',
        email: message.email || 'No email provided',
        message: message.message || 'No message content',
        date: message.created_at,
        isRead: false // Default value for new messages
      }));
  };
        ...
        <div className="mb-8 animate-[fadeInUp_0.6s_ease-out]">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <button 
              ...
              className="bg-gradient-to-r from-[#3567c3] to-[#2a5298] text-white px-6 py-3 rounded-xl font-semibold flex items-center gap-2 hover:shadow-lg transition-all"
            >
              🔄 Refresh
            </button>
          </div>
