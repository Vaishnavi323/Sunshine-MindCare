...
const AdminEnquiries = () => {
  ...
  // API function to fetch enquiries
  const fetchEnquiries = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${API_BASE_URL}/enquires/list`, {
        method: 'GET',
        headers: {
      ...
      if (result.status && result.data) {
        ...
        const transformedEnquiries = transformEnquiriesData(result.data);
      ...
      setAlert({
        type: "error",
        message: error.message || "Failed to fetch enquiries. Please try again.",
      });
    } finally {
      setLoading(false);
    }
  ...
  // Transform API response to match your component structure
  const transformEnquiriesData = (apiData) => {
    return apiData.map(enquiry => ({
      id: enquiry.id,
      name: enquiry.full_name,
      email: enquiry.email,
      age: parseInt(enquiry.age) || 0,
      collegeName: enquiry.college_name,
      studyYear: enquiry.study_year,
      purpose: enquiry.purpose,
      trainingHours: enquiry.training_hours,
      previousInternship: enquiry.previous_internship === "Yes",
      date: enquiry.created_at,
      isRead: false // Default value for new enquiries
    }));
  };
        ...
        <div className="mb-8">
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            ...
            <button 
              ...
              className="bg-gradient-to-r from-[#1f1f35] to-[#174593] text-white px-6 py-3 rounded-xl font-semibold flex items-center gap-2 hover:shadow-lg transition-all"
            >
              🔄 Refresh
            </button>
          </div>
            ...
            {selectedEnquiry ? (
              <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden sticky top-8">
                ...
                <div className="p-6 space-y-5">
                  ...
                  {selectedEnquiry.trainingHours && (
                    <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                      <p className="text-xs font-semibold text-gray-400 uppercase mb-1">Training Hours</p>
                      <p className="text-base font-semibold text-[#1f1f35]">{selectedEnquiry.trainingHours}</p>
                    </div>
                  )}
          ...
          <div className="lg:hidden fixed inset-0 bg-black/50 z-50 flex items-end" onClick={() => setSelectedEnquiry(null)}>
            ...
            >
              ...
              <div className="p-5 overflow-y-auto max-h-[60vh] space-y-4">
                ...
                {selectedEnquiry.trainingHours && (
                  <div className="bg-gray-50 rounded-lg p-3">
                    <p className="text-xs text-gray-400 uppercase">Training Hours</p>
                    <p className="font-semibold text-[#1f1f35]">{selectedEnquiry.trainingHours}</p>
                  </div>
                )}
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-xs text-gray-400 uppercase mb-1">Experience</p>
